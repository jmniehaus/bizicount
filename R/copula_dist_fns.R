...
# 
# cop.pmf=function(margin.args, dep, margins=c("pois", "pois"), cop="frank", pmf.min = 1e-7, bounds=T, frech.min=1e-7){
#   u = mapply(function(margin, args)
#     do.call(paste0("p", margin), args),
#     margin =margins,
#     args =margin.args,
#     SIMPLIFY = F)
#   
#   margin.args[[1]][["q"]] = margin.args[[1]][["q"]] - 1
#   margin.args[[2]][["q"]] = margin.args[[2]][["q"]] - 1
#   
#   uneg =  mapply(function(margin, args)
#     do.call(paste0("p", margin), args),
#     margin =margins,
#     args =margin.args,
#     SIMPLIFY = F)
#   
#   u = mapply(function(ux, umin) .5*(umin+ux), ux=u, umin=uneg, SIMPLIFY=F)
#   
# 
# 
#   # umin = lapply(umin, function(x) as.vector(x))
#   # 
#   # u = mapply(function(mx, mn) ifelse(mn==mx, mn, runif(length(mx), mn, mx)), mx = u, mn = umin, SIMPLIFY = F)
#   # 
#   # u = lapply(u, function(x) as.vector(x))
#   
#   edep = exp(dep)
#   esum = exp(dep*(u[[1]]+u[[2]]))
#   
#   #cat("iterating\n")
#   d =  (dep*esum*(edep-1))/((edep - edep^u[[1]] - edep^u[[2]] + esum)^2)
#   return(d)
#   
# }
